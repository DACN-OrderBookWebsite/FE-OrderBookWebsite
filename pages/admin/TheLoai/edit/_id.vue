...
export default {
  ...
  methods: {
    ...
    async fetch() {
      try {
        const response = await TheLoaiService.getItem(this.$axios, this.$route.params.id);
        this.data = response;
      } catch (error) {
        console.error(error);
      }
    ...
    async update() {
      try {
        await TheLoaiService.update(this.$axios, this.$route.params.id, this.data);
        Swal.fire(
          'Cập nhật!',
          'Thông tin đã được cập nhật thành công.',
          'success'
        );
        this.$router.push('/admin/TheLoai');
      } catch (error) {
