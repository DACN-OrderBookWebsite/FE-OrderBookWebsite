            ...
            <b-col md="6">
              <b-card class="mt-5">
                <b-card-title class="text-center"
                  >Chỉnh Sửa Nhóm người dùng</b-card-title
                >
                <b-form @submit.prevent="confirmUpdate">
                  ...
                  <b-form-group label="Người dùng:" label-for="input-role">
                    <b-form-select
                      id="input-role"
                      v-model="data.idNguoiDung"
                      required
                      :options="NguoiDungOption"
                    ></b-form-select>
                    <small v-if="dataerror.name" class="text-danger">{{
...
export default {
  ...
  methods: {
    ...
    async fetch() {
      try {
        const response = await NhomNguoiDungService.getItem(
          this.$axios,
    ...
    async update() {
      try {
        await NhomNguoiDungService.update(
          this.$axios,
          this.$route.params.id,
          this.data
        );
    ...
    async getEdit() {
      try {
        const response = await NhomNguoiDungService.getEdit(
          this.$axios,
          this.$route.params.id
        );
