...
export default {
  ...
  methods: {
    ...
    async fetch() {
      try {
        const response = await NhomService.getData(this.$axios);
    ...
    add() {
      this.$router.push("/admin/Nhom/create");
    },
    edit(id) {
      this.$router.push(`/admin/Nhom/edit/${id}`);
    },
    ...
    async remove(id) {
      try {
        await NhomService.delete(this.$axios, id);
        await this.fetch();
