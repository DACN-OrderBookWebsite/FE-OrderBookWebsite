    ...
    <b-row>
      ...
      <b-col cols="12" md="10">
        <div id="table_content">
          ...
          <small v-if="dataerror.idNhaCungCap" class="text-danger">{{
            dataerror.idNhaCungCap
          }}</small>
...
export default {
  ...
  methods: {
    ...
    async fetch(TrangThai) {
      try {
        const response = await PhieuNhapService.getDataByidTrangThai(
          this.$axios,
          TrangThai
        );
            ...
            .then((response) => {
              if (response && response.data && Array.isArray(response.data)) {
                response.data.forEach((element) => {
                  console.log(element);
                  this.updateSoLuongSanPhamByPhieuNhap(
                    element.idSanPham,
                    element.SoLuong
                  );
                });
