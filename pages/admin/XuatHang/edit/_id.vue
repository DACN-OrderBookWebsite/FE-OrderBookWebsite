        ...
        <div id="table_content">
          ...
          <div class="user-create-container border">
            <div class="h1 text-center">
              Cập nhật hóa đơn số {{ this.$route.params.id }}
            </div>
            <b-form @submit.prevent="confirmUpdate">
              ...
              >
                <b-form-datepicker
                  ...
                  :value="selectedDate"
                  :disabled="dataHoaDon.idTrangThai === 4"
                ></b-form-datepicker>
              ...
              >
                <span small class="text-danger">{{
                  formatTrangThai(dataHoaDon.idTrangThai)
                }}</span>
              </b-form-group>
              ...
              >
                ...
                <template #cell(actions)="dataSanPham">
                  <b-button
                    ...
                    :disabled="dataHoaDon.idTrangThai !== 1"
                    >Thêm</b-button
                  >
                </template>
              </b-table>
            ...
            <div class="right-table">
              ...
              >
                ...
                <template #cell(DonGia)="dataChiTiet">
                  <b-form-input
                    v-model="dataChiTiet.item.DonGia"
                    type="number"
                    min="0"
                    disabled
                  ></b-form-input>
...
export default {
  ...
  methods: {
    ...
    async updateTongSoLuong_TongTien() {
      const response = await ChiTietHoaDonService.sumSoLuongOfHoaDon(
        this.$axios,
        this.$route.params.id
      );
      this.dataHoaDon.TongSoLuong = response.data.TongSoLuong;
      const response2 = await ChiTietHoaDonService.calculateTongTienOfHoaDon(
        this.$axios,
        this.$route.params.id
      );
    ...
    async fetchHoaDon() {
      try {
        const response = await HoaDonService.getItem(
          this.$axios,
          this.$route.params.id
        );
        this.dataHoaDon = response;
        this.selectedDate = moment(this.dataHoaDon.NgayNhanHang).format(
          "YYYY-MM-DD"
        );
        this.selectedTime = moment(this.dataHoaDon.NgayNhanHang).format(
          "hh:mm:ss"
        );
      } catch (error) {
        console.error(error);
    ...
    async fetchChiTiet() {
      try {
        const response =
          await ChiTietHoaDonService.getDataByidHoaDonAndNameOfSanPham(
            this.$axios,
            this.$route.params.id
          );
    ...
    async insert(item) {
      ...
      try {
        const response =
          await ChiTietHoaDonService.getDataByCheckSanPhamIsInsertedToHoaDon(
            this.$axios,
            this.$route.params.id,
            item.id
          );
        if (response.data.length != 0) {
          ...
          await ChiTietHoaDonService.update(
            this.$axios,
